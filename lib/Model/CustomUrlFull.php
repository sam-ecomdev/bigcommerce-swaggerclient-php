...
class CustomUrlFull implements ModelInterface, ArrayAccess
{
    ...
    public function setUrl($url)
    {
        if (!is_null($url) && (mb_strlen($url) > 255)) {
            throw new \InvalidArgumentException('invalid length for $url when calling CustomUrlFull., must be smaller than or equal to 255.');
        }
        if (!is_null($url) && (mb_strlen($url) < 0)) {
            throw new \InvalidArgumentException('invalid length for $url when calling CustomUrlFull., must be bigger than or equal to 0.');
        }
